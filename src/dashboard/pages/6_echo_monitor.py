...
st.set_page_config(page_title="エコー監視 - GEOスコアリング", page_icon=None, layout="wide", initial_sidebar_state="expanded")
...
if echo_data:
    ...
    if echo_success == '成功':
        status_gradient = f"linear-gradient(135deg, {COLORS['success']} 0%, #86efac 100%)"
        status_icon = "check_circle"
    elif echo_success == '部分成功':
        status_gradient = f"linear-gradient(135deg, {COLORS['warning']} 0%, #fcd34d 100%)"
        status_icon = "zap"
    else:
        status_gradient = f"linear-gradient(135deg, {COLORS['danger']} 0%, #fca5a5 100%)"
        status_icon = "alert_triangle"
    ...
    ">
        <div style="display: flex; align-items: center; gap: 1rem;">
            ...
            ">
                {get_icon(status_icon, size=24, color='white')}
            </div>
            <div>
    ...
    with col1:
        ...
        <div style="
            ...
            text-align: center;
        ">
            <div style="color: {COLORS['accent_secondary']}; margin-bottom: 0.5rem;">
                {get_icon('activity', size=24, color=COLORS['accent_secondary'])}
            </div>
            <div style="font-size: 0.875rem; color: {COLORS['text_secondary']};">メンション数</div>
            <div style="font-size: 2.5rem; font-weight: 700; color: {COLORS['accent_secondary']}; font-family: 'JetBrains Mono', monospace; margin: 0.25rem 0;">
                {mention_count}
            </div>
    ...
    with col2:
        ...
        <div style="
            ...
            text-align: center;
        ">
            <div style="color: {sent_color}; margin-bottom: 0.5rem;">
                {get_icon('message', size=24, color=sent_color)}
            </div>
    ...
    with col3:
        ...
        <div style="
            ...
            text-align: center;
        ">
            <div style="color: {rank_color}; margin-bottom: 0.5rem;">
                {get_icon('target', size=24, color=rank_color)}
            </div>
    ...
    with col4:
        ...
        ">
            ...
            <div style="font-size: 2.5rem; font-weight: 700; color: {COLORS['text_primary']}; font-family: 'JetBrains Mono', monospace; margin: 0.25rem 0;">
    ...
    ">
        <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
            <div style="color: {COLORS['accent_tertiary']};">
                {get_icon('brain', size=20, color=COLORS['accent_tertiary'])}
            </div>
            <span style="font-size: 1rem; font-weight: 600; color: {COLORS['text_primary']};">LLM別カバレッジ</span>
        </div>
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem;">
    """, unsafe_allow_html=True)

    llm_icons = {'ChatGPT': 'brain', 'Gemini': 'activity', 'Claude': 'layers', 'Perplexity': 'eye'}

    for llm_name, is_active in llm_coverage.items():
        bg_color = COLORS['success'] if is_active else COLORS['card_elevated']
        text_color = 'white' if is_active else COLORS['text_secondary']
        status_text = "引用あり" if is_active else "引用なし"
        icon_name = llm_icons.get(llm_name, 'brain')

        st.markdown(f"""
        <div style="
            background: {bg_color if is_active else COLORS['card_elevated']};
            border-radius: 12px;
            padding: 1rem;
            text-align: center;
            {'border: 2px solid ' + COLORS['success'] if is_active else 'border: 1px solid ' + COLORS['border']};
        ">
            <div style="margin-bottom: 0.5rem;">
                {get_icon(icon_name, size=24, color=text_color if is_active else COLORS['text_muted'])}
            </div>
    ...
    with tab1:
        ...
        fig_timeline.add_trace(go.Scatter(
            ...
            fill='tozeroy',
            fillcolor='rgba(222, 255, 154, 0.1)',
        ...
        # 参照順位推移
        st.markdown(f"""
        <div style="margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid {COLORS['border']};">
    ...
    with tab2:
            ...
            <div style="margin-bottom: 1rem;">
                <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem;">
                    <div style="color: {COLORS['success']};">
                        {get_icon('message', size=20, color=COLORS['success'])}
                    </div>
        ...
        # センチメント推移
        st.markdown(f"""
        <div style="margin-top: 1.5rem; padding-top: 1.5rem; border-top: 1px solid {COLORS['border']};">
            <div style="font-size: 1rem; font-weight: 600; color: {COLORS['text_primary']}; margin-bottom: 1rem;">
                センチメント推移
            </div>
        </div>
            ...
            for citation in recent_citations:
                ...
                ">
                    ...
                    <div style="margin-top: 0.75rem; font-size: 0.8125rem;">
                        ...
                        <a href="{source_url}" target="_blank" style="color: {COLORS['accent_secondary']}; text-decoration: none;">
                            {source_url}
                        </a>
                    </div>
                </div>
    ...
    with tab4:
        ...
        with col_stat2:
            ...
            fig_rank_dist = go.Figure(data=[
                go.Bar(
                    ...
                    marker=dict(
                        color=[COLORS['success'], COLORS['warning'], COLORS['accent_secondary']],
                        cornerradius=8
                    ),
        ...
        for criteria, met in success_criteria:
            icon_name = "check_circle" if met else "alert_triangle"
            bg_color = f"rgba(74, 222, 128, 0.1)" if met else f"rgba(251, 191, 36, 0.1)"
            text_color = COLORS['success'] if met else COLORS['warning']
            ...
            ">
                <div style="color: {text_color};">
                    {get_icon(icon_name, size=20, color=text_color)}
                </div>
    ...
    ">
        <div style="
            ...
            margin: 0 auto 1rem;
        ">
            {get_icon('eye', size=40, color=COLORS['text_muted'])}
        </div>
