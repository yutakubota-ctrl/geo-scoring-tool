...
if own_brand and latest_score and avg_scores:
    ...
    if not is_demo_mode_active():
        ...
        trend_data = [
            {"date": (datetime.now() - timedelta(days=i)).strftime("%Y-%m-%d"), "total": 70 + i * 0.1}
    ...
    with col_left:
        ...
        if is_demo_mode_active():
            # デモモード: ダミーランキングデータ
            ranking_data = [
                {"brand": "HubSpot", "score": 72, "is_own": True},
                {"brand": "Salesforce", "score": 75, "is_own": False},
                {"brand": "Marketo", "score": 68, "is_own": False},
                {"brand": "Pardot", "score": 65, "is_own": False}
            ]
        else:
            with db.get_session() as session:
